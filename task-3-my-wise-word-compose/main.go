...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<h2>Wise Words</h2><p>Learning never exhausts the mind.</p>")
}

// Fungsi untuk mendapatkan koneksi ke database
func getDBConnection() (*sql.DB, error) {
	// Mengambil konfigurasi koneksi dari environment variables
	dbHost := os.Getenv("DB_HOST")
	dbUser := os.Getenv("DB_USER")
	dbPass := os.Getenv("DB_PASSWORD")
	dbName := os.Getenv("DB_NAME")

	// Set nilai default jika environment variables tidak di-set
	if dbHost == "" {
		dbHost = "postgresdb"
	}
	if dbUser == "" {
	...
	// Connection string ke database Postgres
	connStr := fmt.Sprintf(
...
func connectHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Insert log akses dengan waktu sekarang
	_, err = db.Exec("INSERT INTO rivanapta_access_log (timestamp) VALUES ($1)", time.Now())
	if err != nil {
